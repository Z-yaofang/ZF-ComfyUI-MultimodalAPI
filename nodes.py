...
def _local_image_frames(images: Iterable[Any], max_image_size: int) -> List[Any]:
    """Flatten ComfyUI IMAGE batches and cap each frame without changing aspect ratio."""
    ...
    try:
        ...
        raise RuntimeError("本地多模态节点需要 ComfyUI 自带的 PyTorch 环境。") from error

    frames: List[Any] = []
    max_image_size = max(128, int(max_image_size))
    for image in images:
        if not torch.is_tensor(image):
            raise TypeError("本地多模态节点的图片输入必须是 ComfyUI IMAGE 张量。")
...
def _local_video_frames(video_frames: Any, max_image_size: int, max_frames: int) -> List[Any]:
    """Sample a ComfyUI IMAGE batch used as a local video's visual evidence.

    The local llama.cpp VLM cannot consume ComfyUI VIDEO/AUDIO objects.  A bounded
    IMAGE batch is therefore accepted as an explicit, visual-only video proxy.  It
    is sampled chronologically and capped before inference so a 15-second 24-fps
    clip cannot accidentally expand into hundreds of image tokens.
    """
    if video_frames is None:
        return []
    max_frames = max(1, int(max_frames))
    # Select the temporal subset before resizing/copying frames to CPU.  This is
    # important for a 15-second 24-fps batch: sampling after materialization would
    # briefly allocate hundreds of resized tensors even though only a few are used.
    ...
    merged.update(
        {
            ...
            # ComfyUI-llama-cpp_vlm currently exposes this spelling.
...
def _resolve_llama_cpp_instruct():
    try:
        comfy_nodes = importlib.import_module("nodes")
        node_class = comfy_nodes.NODE_CLASS_MAPPINGS.get("llama_cpp_instruct_adv")
    except Exception as error:
        raise RuntimeError(
            "无法读取 ComfyUI 节点注册表。请确认在 ComfyUI 内运行本节点。"
        ) from error
...
class ZFLocalMultimodalLLM:
    DESCRIPTION = (
        "使用本机 llama.cpp VLM 完成文本/多图反推，不发送网络请求。"
        "依赖 ComfyUI-llama-cpp_vlm；最多接 8 路图片，图片会等比缩小但不会裁剪或拉伸。"
    )
    RETURN_TYPES = ("STRING", "STRING")
    RETURN_NAMES = ("response", "raw_response")
    FUNCTION = "chat"
            ...
            "required": {
                ...
                "max_image_size": (
                    "INT",
                    {
                        ...
                        "max": 4096,
                        "step": 64,
                        "tooltip": "每张图最长边的上限。保持原始宽高比，不裁剪、不补黑边。",
                    },
                ),
                "seed": (
                    "INT",
                ...
                "force_offload": (
                    ...
                    {
                        "default": False,
                        "tooltip": "完成后卸载本地 VLM；接下来要运行大型视频模型时建议开启。",
                    },
                ),
            },
            "optional": {
                ...
                "parameters": (
                    "LLAMACPPARAMS",
                    {
                        "tooltip": "可选高级参数；本节点上的温度、长度、top_p 和惩罚值优先。",
                    },
                ),
                "image1": ("IMAGE",),
                "image2": ("IMAGE",),
                ...
                "video_frames": (
                    "IMAGE",
                    {
                        "tooltip": "可选：把视频抽帧后的 IMAGE 批次作为视觉证据；本地模型只看画面，不读取视频音轨。",
                    },
                ),
                "video_max_frames": (
                    "INT",
                    {
                        ...
                        "max": 32,
                        "step": 1,
                        "tooltip": "视频代理最多送入多少帧；按时间均匀抽样，避免把整段 24fps 视频全部送入本地模型。",
                    },
                ),
            },
            "hidden": {
    ...
    def _error_result(self, error: Exception):
        message = f"[ERROR] ZFLocalMultimodalLLM：{error}"
        raw = json.dumps(
        ...
        try:
            ...
            if video_proxy_frames:
                media_note = (
                    ...
                    f"[<Video 1> local visual proxy: {len(video_proxy_frames)} uniformly sampled frames; "
                    "visual-only; synchronized audio was not analyzed.]"
                )
            effective_prompt = prompt or ""
            if media_note:
                effective_prompt = f"{effective_prompt}\n\n{media_note}"
            ...
            output, _output_list, state_uid = backend.process(
                ...
                save_states=False,
                unique_id=str(unique_id or "zf-local-multimodal"),
                parameters=local_parameters,
                images=(frames + video_proxy_frames) or None,
                queue_handler=None,
            )
            text = _clean_text(str(output))
            if not text:
            ...
            if skip_error:
                log.error("ZFLocalMultimodalLLM failed: %s", str(error))
                return self._error_result(error)
            raise
